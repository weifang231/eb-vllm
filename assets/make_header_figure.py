...
# Color functions
def th_color(g):
    if g > 5:   return "#2E8B57"
    if g > -3:  return "#B0B0B0"
    return "#C04A4A"
def tpot_color(t):
    if t > 10:  return "#2E8B57"   # big EB win
    if t > 1:   return "#7BB892"   # small EB win
    return "#B0B0B0"

# ---------------- LEFT PANEL: Throughput ----------------
bars_th = axL.bar(x, tps, color=[th_color(g) for g in tps],
...
bars_tpot = axR.bar(x, tpots, color=[tpot_color(t) for t in tpots],
                     edgecolor="black", linewidth=0.7, width=0.62)
...
for i, (bar, t) in enumerate(zip(bars_tpot, tpots)):
    h = bar.get_height()
    axR.text(bar.get_x() + bar.get_width()/2, h + 1.0, f"−{t:.1f}%",
             ha="center", va="bottom", fontsize=12, fontweight="bold")
