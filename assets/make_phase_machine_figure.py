...
ax.annotate("", xy=(mid_decode_x, N - k_hat + 0.5),
            xytext=(mid_decode_x, N - 0.5),
            arrowprops=dict(arrowstyle="<->", color="#444", lw=1.4), zorder=4)
ax.text(mid_decode_x + 1, N - k_hat / 2, r"$\hat{k}^*$ completions",
        fontsize=11.5, ha="left", va="center", color="#222",
        bbox=dict(boxstyle="round,pad=0.25", facecolor="white",
                  edgecolor="#888", alpha=0.95), zorder=5)

# Phase labels (above plot, in each region of cycle 1)
ax.text((phases[0][0] + phases[0][1]) / 2, N + 7,
        "Phase 1\nDecode", ha="center", fontsize=10.5, color="#2E8B57",
        fontweight="bold")
ax.text((phases[1][0] + phases[1][1]) / 2, N + 7,
        "Phase 2\nRefill", ha="center", fontsize=10.5, color="#B07424",
        fontweight="bold")
...
for i, (t0, t1, kind) in enumerate(phases[2:], start=2):
    ...
    ax.text((t0 + t1) / 2, N + 4, label, ha="center", fontsize=9.5,
...
# Title
ax.set_title("EB inventory dynamics: batch oscillates between "
             r"$N$ and $N - \hat{k}^*$",
             fontsize=13, fontweight="bold", pad=12)

# Legend (phase shading)
decode_patch = mpatches.Patch(color="#2E8B57", alpha=0.25,
                               label="Phase 1: Decode  (advance all active requests)")
refill_patch = mpatches.Patch(color="#E8A33D", alpha=0.30,
                               label=r"Phase 2: Refill  (prefill $\hat{k}^*$ new requests)")
ax.legend(handles=[decode_patch, refill_patch], loc="lower right",
          fontsize=10.5, framealpha=0.95, edgecolor="#888")
