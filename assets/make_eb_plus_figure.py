...
Visualizes that EB+ (hybrid scheduler) never loses to v1 (MB), while
EB(k̂*) alone can lose by ~25% in low-load regimes. Data from paper
...
data = [
    ...
    ("c=32",          "RTX 6000", -21.6, -0.4),   # Table 4
    ("c=32",          "H200",     -25.7,  0.0),
    ("c=2048",        "RTX 6000", +35.0, +38.1),
    ("c=2048",        "H200",     +3.1,  +2.6),
    # Non-stationary
    ("Distrib. shift", "RTX 6000", +29.3, +37.5),  # Table 5
    ("Distrib. shift", "H200",     -5.0,  +13.5),
    ("Concur. shift",  "RTX 6000", +9.5,  +14.3),
...
eb_bars = ax.bar(x - width/2, eb_gains, width,
                  label=r"EB($\hat{k}^*$)", color="#8FA8C9",
                  edgecolor="black", linewidth=0.6)

# EB+ bars: highlight gold
ebp_bars = ax.bar(x + width/2, ebp_gains, width,
                   label=r"EB$^+$ (hybrid)", color="#E8A33D",
...
for bar, v in zip(eb_bars, eb_gains):
    ...
    ax.text(bar.get_x() + bar.get_width()/2, y, f"{v:+.1f}",
            ha="center", va=va, fontsize=8.5, color="#445")
...
for bar, v in zip(ebp_bars, ebp_gains):
    ...
    va = "bottom" if h >= 0 else "top"
    ax.text(bar.get_x() + bar.get_width()/2, y, f"{v:+.1f}",
            ha="center", va=va, fontsize=9, fontweight="bold", color="#7a4a00")
...
# and between c=32/c=2048
for sep in (1.5, 3.5, 5.5):
    ax.axvline(sep, color="#cccccc", linewidth=0.7, linestyle="--", zorder=0)

# Group labels at top
ax.text((0+1)/2, 62, "Low load\n(c=32)", ha="center", fontsize=10,
        color="#444", style="italic")
ax.text((2+3)/2, 62, "High load\n(c=2048)", ha="center", fontsize=10,
        color="#444", style="italic")
ax.text((4+5)/2, 62, "Distribution shift\n(non-stationary)", ha="center",
        fontsize=10, color="#444", style="italic")
ax.text((6+7)/2, 62, "Concurrency shift\n(non-stationary)", ha="center",
        fontsize=10, color="#444", style="italic")

# Y axis
ax.set_ylabel("Throughput change vs v1 (MB)  (%)", fontsize=11.5)
ax.set_ylim(-35, 72)
ax.set_yticks(range(-30, 51, 10))
ax.grid(axis="y", linestyle=":", alpha=0.4, zorder=0)

# Legend
ax.legend(loc="lower right", fontsize=10.5, framealpha=0.95,
          edgecolor="#888", ncol=2)

# (No headline annotation box — title + data labels + group labels suffice.)

# Title
ax.set_title("EB$^+$: hybrid scheduler that adaptively picks "
             "the better of {EB, MB}\n"
             r"$\it{Qwen3\!-\!8B,\ paper\ Tables\ 4\ and\ 5}$",
             fontsize=13, fontweight="bold", pad=10, linespacing=1.3)
