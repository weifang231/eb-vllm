...
(Full Iteration Version - measures entire iteration including schedule/update)
...
Unlike calibration.py which only measures execute_model() time, this version
measures the ENTIRE iteration time including:
  - scheduler.schedule()
...
class HardwareCalibratorFullIter:
    """Calibrates hardware parameters by running benchmarks.

    This version measures the FULL iteration time including:
    - scheduler.schedule()
    - execute_model()
    - sample_tokens() (if needed)
    - scheduler.update_from_output()
    """
    ...
    def _run_single_step_full_iter(self) -> tuple[int, int, int, float]:
        """Run a single step and measure FULL ITERATION time.

        Unlike the original version, this measures:
        - scheduler.schedule()
        - execute_model()
        - sample_tokens() (if needed)
        - scheduler.update_from_output()
    ...
    def measure_prefill_times(self) -> list[tuple[int, float]]:
        """Measure prefill times for different input sizes (full iteration).
    ...
    def measure_decode_times(self) -> list[tuple[int, float]]:
        """Measure decode times for different batch sizes (full iteration).
    ...
    def calibrate(self) -> HardwareParams:
        ...
        params = HardwareParams(
            ...
            measurement_type="full_iteration",
            prefill_r2=r2_p,
            decode_r2=r2_d,
...
def calibrate_hardware_params_full_iter(
    model: str,
    dtype: str = "float16",
    prefill_sizes: list[int] | None = None,
    decode_counts: list[int] | None = None,
    ...
    """
    Convenience function to run calibration (full iteration version).
    ...
    """
    calibrator = HardwareCalibratorFullIter(
        model=model,
        dtype=dtype,
...
def main():
    ...
    params = calibrate_hardware_params_full_iter(
        model=args.model,
        dtype=args.dtype,
        prefill_sizes=prefill_sizes,
        decode_counts=decode_counts,
    ...
    print("\n" + "=" * 60)
    print("CALIBRATION RESULTS (Full Iteration)")
    print("=" * 60)
    print(params)
    print("=" * 60)
    print("\nThis version measures the ENTIRE iteration time including:")
